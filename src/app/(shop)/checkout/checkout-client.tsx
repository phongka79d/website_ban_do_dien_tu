...
const checkoutSchema = z.object({
  shippingAddress: z.string()
    .min(12, "Vui lòng nhập địa chỉ giao hàng rõ ràng (ít nhất 12 ký tự)")
    .max(300, "Địa chỉ quá dài, vui lòng rút gọn lại (tối đa 300 ký tự)"),
  phoneNumber: z.string()
    .regex(/^0\d{8,10}$/, "Số điện thoại không hợp lệ (Phải từ 9-11 chữ số)"),
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="relative z-10 flex flex-col items-center">
              <motion.div
                ...
                className={`w-11 h-11 rounded-full flex items-center justify-center text-sm font-black transition-all ring-4 ${step >= s
              ...
              <span className={`absolute -bottom-10 text-[11px] font-black uppercase tracking-[0.15em] whitespace-nowrap transition-all duration-300 ${step >= s ? "text-slate-900" : "text-slate-300"
                }`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative group/phone">
                          <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.phoneNumber ? "text-red-400" : "text-slate-400 group-focus-within/phone:text-primary"
                            }`}>
                            <Phone size={18} />
                          </div>
                          <input
                            ...
                            className={`w-full h-12 pl-12 pr-4 rounded-xl bg-white border outline-none transition-all duration-300 font-bold ${errors.phoneNumber
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          className={`w-full p-4 rounded-xl bg-white border outline-none transition-all duration-300 font-bold ${errors.shippingAddress
                              ? "border-red-500 focus:ring-4 focus:ring-red-500/10 shadow-[0_0_0_1px_rgba(239,68,68,0.2)]"
                              : "border-slate-200 focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 shadow-sm"
                            }`}
                        />
              ...
              >
                <Card radius="2xl" className="p-8 border-white shadow-xl bg-white/80 backdrop-blur-sm">
                  ...
                  <div className="space-y-4">
                    <label className={`flex items-center gap-5 p-6 rounded-3xl border-2 transition-all cursor-pointer relative overflow-hidden group/payment ${formData.paymentMethod === 'cod' ? "border-pink-500 bg-pink-50/30" : "border-slate-100/50 hover:border-slate-200 bg-slate-50/30"
                      }`}>
                    ...
                    <label className={`flex items-center gap-4 p-5 rounded-2xl border-2 transition-all cursor-pointer opacity-60 ${formData.paymentMethod === 'BANK' ? "border-primary bg-primary/5" : "border-slate-100 hover:border-slate-200"
                      }`}>
                      <input
                        type="radio"
                        name="paymentMethod"
